...
class TestTransSimplify:
    ...
    def test_bool_ops(self):
        ...
        assert str(self.transform(expr)) == "[]"
    ...
    def test_boolvar_comps(self):
        ...
        for op in test_dict:
            for rhs, val_should in test_dict[op].items():
                ...
                print(expr)
                if val_should is True:
                    str_val_should = "[]"
                elif val_should is False:
                    str_val_should = "[boolval(False)]"
                else:
                    str_val_should = str([val_should])

                assert str(self.transform(expr)) == str_val_should
    ...
    # # issue #322
    # def test_with_floats(self): -> Deprecated, we don't support floats anymore
    #     expr = self.bvs[0] == 1.0
    #     assert str(self.transform(expr)) == "[bv[0]]"
