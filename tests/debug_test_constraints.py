...
def run_bool_constraints():

    for solver, constraint in _generate_inputs(bool_exprs):
        print(solver, "-", constraint)
        assert SolverLookup.get(solver, Model(constraint)).solve(proof=None)
        assert argval(constraint)
        assert constraint.value()

        print(solver, "-", constraint, "with proof")
        assert SolverLookup.get(solver, Model(constraint)).solve(proof="True")
        assert argval(constraint)
        assert constraint.value()


def run_comparison_constraints():

    for solver, constraint in _generate_inputs(comp_constraints):
        print(solver, "-", constraint)
        assert SolverLookup.get(solver, Model(constraint)).solve(proof=None)
        assert argval(constraint)
        assert constraint.value()
...
def run_reify_imply_constraints():

    for solver, constraint in _generate_inputs(reify_imply_exprs):
        print(solver, "-", constraint)
        assert SolverLookup.get(solver, Model(constraint)).solve(proof=None)
        assert argval(constraint)
        assert constraint.value()
...
if __name__ == "__main__":
    ...
    run_bool_constraints()
    run_comparison_constraints()
    run_reify_imply_constraints()