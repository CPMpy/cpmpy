"""
Nurse Rostering as a CPMpy benchmark

This module provides a benchmarking framework for running CPMpy on Nurse Rostering 
instances from schedulingbenchmarks.org.

Command-line Interface
----------------------
This script can be run directly to benchmark solvers on Nurse Rostering datasets.

Usage:
    python nurserostering.py --solver ortools
...
def solution_nurserostering(model):
    """
    ...
    """
    variables = {var.name: var.value() for var in model.user_vars if var.name[:2] not in ["IV", "BV", "B#"]}
    return str(variables)


class NurseRosteringBenchmark(Benchmark):
    ...
    def __init__(self):
        self.sol_time = None
        super().__init__(reader=read_nurserostering)
    ...
    def print_result(self, s):
        if s.status().exitstatus == CPMStatus.OPTIMAL:
            ...
            self.print_value(solution_nurserostering(s))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Benchmark solvers on Nurse Rostering instances')
    ...
    from cpmpy.tools.dataset.problem.nurserostering import NurseRosteringDataset
    dataset = NurseRosteringDataset(root=".", download=True)
    ...
    output_file = str(output_dir / "nurserostering" / f"nurserostering_{args.solver}_{timestamp}.csv")

    # Run the benchmark
    instance_runner = NurseRosteringBenchmark()
