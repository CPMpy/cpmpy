...
class Runner:
    ...
    def run(self, instance: str, solver: Optional[str] = None, time_limit: Optional[int] = None, mem_limit: Optional[int] = None, seed: Optional[int] = None, intermediate: bool = False, cores: int = 1, **kwargs):
        ...
        _sys.setrecursionlimit(max(_old_rlimit, 50_000))
    ...
    def print_forwarding_context(self):
        ...
        class PrintForwarder:
            ...
            def write(self, text):
                if getattr(self.runner, '_in_observer_print', False):
    ...
    def observe_intermediate(self, objective, elapsed_seconds=None):
        self._in_observer_print = True
        try:
            for observer in self.observers:
                observer.observe_intermediate(runner=self, objective=objective, elapsed_seconds=elapsed_seconds)
        finally:
            self._in_observer_print = False
