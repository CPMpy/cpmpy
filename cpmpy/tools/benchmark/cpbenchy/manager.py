...
# from ..benchmarks.xcsp3.run_xcsp3_instance import XCSP3InstanceRunner
from .observers import ResourceLimitObserver

def _mib_as_bytes(mib: int) -> int:
    return mib * 1024 * 1024


def _append_option_if_missing(cmd: list, flag: str, value=None, is_bool: bool = False) -> None:
    """Append a CLI option unless the runner-specific command already supplied it."""
    if flag in cmd:
        return
    if is_bool:
        if value:
...
class RunExecResourceManager(ResourceManager):
    """
    ...
    def run(self, 
        ...
        if output_file is not None:
            ...
            from .observers import WriteToFileObserver
        ...
        try:
            ...
            with self._print_forwarding_context(runner):
                ...
                cores_count = len(cores) if cores else None
                try:
                    cmd = runner.cmd(
                        instance,
                        solver=solver,
                ...
                _append_option_if_missing(cmd, "--solver", solver)
                _append_option_if_missing(cmd, "--output_file", output_file)
                _append_option_if_missing(cmd, "--time_limit", time_limit)
                _append_option_if_missing(cmd, "--mem_limit", memory_limit)
                _append_option_if_missing(cmd, "--seed", seed)
                _append_option_if_missing(cmd, "--cores", cores_count)
                _append_option_if_missing(cmd, "--intermediate", intermediate, is_bool=True)
                _append_option_if_missing(cmd, "--verbose", verbose, is_bool=True)
                ...
                result = executor.execute_run(
                        ...
                        cores=cores,
                        memlimit=_mib_as_bytes(memory_limit) if memory_limit is not None else None,
                ...
                with open(tmp_filename, 'r', encoding='utf-8', errors='replace') as f:
                    for line in f:
                        ...
                        if line_stripped and not _is_runexec_message(line_stripped):
                            # Subprocess output is already formatted by the child runner.
                            # Echo it raw to avoid turning "c ..." into "c c ...".
...
def load_instance_runner(runner_path: str) -> InstanceRunner:
    ...
    if ":" in runner_path:
        ...
        sys.modules[module_name] = module  # register so inspect.getfile() can find the source
