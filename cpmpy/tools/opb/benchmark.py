"""
Benchmark solvers on OPB PB24 Instances by replicating the PB competition.

A command-line tool for benchmarking constraint solvers on PB competition instances.
Supports parallel execution, time/memory limits, and solver configuration.

Required Arguments
------------------
--year : int
    The competition year (e.g., 2024).

--track : str
    The competition track (e.g., "DEC-LIN", "OPT-LIN", "DEC-NLC").
...
--verbose
    If set, display full opb output during execution.
...
from cpmpy.tools.opb.dataset import OPBDataset
from cpmpy.tools.opb.opb_cpmpy import opb_cpmpy, init_signal_handlers, ExitStatus
...
def opb_wrapper(conn, kwargs, verbose):
    """
    Wraps a call to opb_cpmpy as to correctly 
...
def execute_instance(args: Tuple[str, dict, str, int, int, int, str, bool, bool, str]) -> None:
    """
    Solve a single OPB instance and write results to file immediately.
    
    Args is a list of:
        filename: Path to the OPB instance file
    ...
    if str(filename).endswith(".lzma"):
        # Decompress the XZ file TODO: put in generic place
    ...
    process = ctx.Process(target=opb_wrapper, args=(
                                                    child_conn, 
                                                      {
                                                          "benchname": filename, 
                                                          "solver": solver, 
    ...
    # TODO No pre-made solution checker exists?
...
def opb_benchmark(year: int, track: str, solver: str, workers: int = 1, 
    ...
    Args:
        year (int): Competition year (e.g., 2024)
        track (str): Track type (e.g., "DEC-LIN", "OPT-LIN", "DEC-NLC")
    ...
    output_file = str(output_dir / f"opb_{year}_{track}_{solver}_{timestamp}.csv")
    
    # Initialize dataset
    dataset = OPBDataset(year=year, track=track, download=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--track', type=str, required=True, help='Track type (e.g., "DEC-LIN", "OPT-LIN", "DEC-NLC")')
    ...
    output_file = opb_benchmark(**vars(args))
