    ...
    for w, vars_str in TERM_RE.findall(line):
        ...
        for v in vars_str.split():
            if v.startswith("~x"):
                ...
                idx = int(v[1:])
                factors.append(vars[idx])
        term = int(w) * reduce(mul, factors, 1)
        terms.append(term)
    return cp.sum(terms)
...
def read_opb(path: Union[str, os.PathLike]) -> cp.Model:
    """
    ...
    Returns:
        cp.Model: A CPMpy Model representing the parsed OPB problem,
    ...
    # Determine if source is a path or a string
    if isinstance(path, (str, os.PathLike)) and os.path.exists(path):
        f_open = lzma.open if str(path).endswith(".xz") else open
        f = f_open(path, 'rt')
    else:
        # Treat as OPB text
        f = StringIO(path)
