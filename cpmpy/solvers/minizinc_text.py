...
from ..expressions.globalconstraints import *
import numpy as np
...
class MiniZincText(SolverInterface):
    ...
    def cleanname(self, varname):
        return varname.replace('[','_').replace(']','')

    def convert(self, model):
        modelvars = get_variables_model(model)
        txt_vars = "% Generated by CPMpy\ninclude \"globals.mzn\";\n\n"
        for var in modelvars:
            if isinstance(var, _BoolVarImpl):
                txt_vars += "var bool: {};\n".format(self.cleanname(var.name))
            elif isinstance(var, _IntVarImpl):
                txt_vars += "var {}..{}: {};\n".format(var.lb, var.ub, self.cleanname(var.name))
        ...
        if not isinstance(expr, Expression) or \
           isinstance(expr, _NumVarImpl):
            ...
            if expr is False:
                return "false"
            # default
            return self.cleanname(str(expr))
