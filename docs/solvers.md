...
The default solver is OR-Tools CP-SAT, an award winning constraint solver. But CPMpy supports multiple other solvers: more CP solvers (IBM-CPO, Optal, Choco, GCS, Pumpkin), ILP solvers (Gurobi, IBM ILOG CPLEX, SCIP, HiGHS), SAT solvers (those in PySAT and Pindakaas), the RC2 MaxSAT solver, the Z3 SMT solver, a conflict-driven cutting-planes solver (Exact), even a knowledge compiler (PySDD), a global optimisation solver (Hexaly) and any CP solver supported by the text-based MiniZinc language.
