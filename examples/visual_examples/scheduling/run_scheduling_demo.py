...
def run():
    ...
    jobs = ['A', 'B', 'C'] # Different jobs
    lastT = 11 # Time limit
    dur = cpm_array([[3, 2, 2 ], [2, 1, 4], [ 4, 3 ]]) # Amount of time needed per task
    taskToMach = cpm_array([[1, 2, 3 ],[ 1, 3, 2],[ 2, 3 ]]) # On what machine each task has to be performed
...
def model_scheduling(jobs, lastT, dur, taskToMach):
    nTasks = max([len(x) for x in dur]) # Amount of tasks per job
    ...
    for i in range(nJobs):
        for j in range(len(taskToMach[i])):
            m += (end[i,j] == start[i,j] + dur[i][j])

    # In every pair of jobs on the same machine, one comes before the other
    for i1 in range(nJobs):
        for i2 in range(nJobs): 
            for j1 in range(len(taskToMach[i1])): 
                for j2 in range(len(taskToMach[i2])):
    ...
    for i in range(nJobs): 
        for j in range(len(taskToMach[i])-1):
    ...
    # Job B has to be started immediately has to come before all 2nd tasks of other jobs
    # for i in range(nJobs):
    #     if i != 1:
    #         m += (start[i,0] >= start[1,0])
...
def visualize_scheduling(vars, lastT, jobs, taskToMach):
    nMachines = max([taskToMach[i][j] for i in range(len(taskToMach)) for j in range(len(taskToMach[i]))]) # Amount of machines present
    ...
    for i,j in enumerate(jobs):
        job_name = str(j)
        for t in range(len(taskToMach[i])):
            ...
            img1.rectangle(shape, fill=task_cs[i], outline=task_border_cs[i])
