...
piece_data = [[-1.1558e+01, -1.8940e-01, -1.0785e+01, -8.4097e+00, -1.3034e+01, -8.7986e+00, -2.6694e+00,  1.2009e+01, -3.5726e+00, -9.1412e-01, -2.0600e+00, -5.9005e-01],
...
occupancy = [False, False, False, False, False, False,  True, False,
...
#a variable to represent the whole board
board = intvar(0, 12, (8, 8), "board")

#a variable to represent the pieces
pieces = intvar(0, 10000000, (2, len(piece_data)), name="pieces")

#make the probabilites into reasonable big values
pieces_values = np.empty([len(piece_data), len(piece_data[0])], dtype=int)
for i in range(len(piece_data)):
    for j in range(len(piece_data[0])):
        pieces_values[i,j] = int((piece_data[i,j]*10000).item())
